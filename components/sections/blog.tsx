    ...
    <div className={styles.post}>
        ...
        {featured && (
            showVideo ? (
                ...
                <iframe
                    ...
                    src={"https://youtube.com/embed/" + video.substr(video.length - 11)} frameBorder="0" allowFullScreen/>
            </div>
        ...
        <Link href={`/blog/[projectSlug]`} as={`/blog/${slug}`}>
            <a aria-label={`${title} - Blog`}>
                {
                    featured && (
                        <footer className={styles.footer}>{!showVideo ? "Keep reading" : "Keep watching"}</footer>
                    )}
                {
                    !featured && (
                        <footer className={styles.footer}>Read</footer>
                    )}
            </a>
        </Link>
